...
if (isset($_POST['submit'])) {
    $fullName = $_SESSION['auctionUser'];
    $message = $_POST['message'];
    // comment of user inserting to database
    $query = "INSERT INTO review (`full_name`, `reviewText`, `auctionId`) VALUES ('$fullName', '$message', '$id')";
    if ($databaseConnection->query($query)) {
        echo 'Comment added';
    }
}
else if (isset($_POST['bidAmt'])) {
    $bidAmount = $_POST['amount'];
    // bid amount inserting to database
...
?>
    <article class="product">
        ...
        <section class="reviews">
            ...
            ?>
                <form method="POST">
                    <label>Comment on this product</label> <textarea required name="message"></textarea>
